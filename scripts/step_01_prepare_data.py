...
def json2img(input_dir, output_dir, auto_split=False):
    input_dir = Path(input_dir)
    output_dir = Path(output_dir)
    for p in tqdm(input_dir.glob('*.json'), desc='Processing'):
        with open(str(p), 'r') as f:
            ...
            # 处理图片
            folder = output_dir.joinpath(cls)
            ...
            if res['label'] > 1:
                ...
                points = ShapeUtil.map_points(points, x, y, w, h, IMAGE_WIDTH, IMAGE_HEIGHT,
                                              scale=SCALING_FACTOR).tolist()
